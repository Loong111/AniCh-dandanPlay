...
- 2026-05-09 fallback hotfix: Bilibili manual imports and cached restores no longer require a loaded Dandanplay base bucket. If Dandanplay automatic matching fails because a new episode is missing upstream, the session attempts Bilibili restore and keeps Bilibili-only sources instead of clearing the store and leaving the page stuck at pending match; if Dandanplay is matched later, its base bucket is moved back to the first source position to preserve base-source dedupe priority.
