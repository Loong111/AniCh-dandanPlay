...
本项目的用途很直接：在 AniCh 上提供一个基于弹弹 Play 的弹幕脚本。它把匹配、加载、过滤、渲染和页面内控制栏整合到一个 userscript 里，方便直接安装和使用。
...
- 支持导入 B 站 BV/视频链接、`b23` 短链和正版番剧 `ep`/`ss` 链接弹幕，并可多链接合并、按同季链自动恢复。
- 提供位于播放器下方的页面内控制栏，避免遮挡视频画面并融入 AniCh 页面布局。
- 支持字号、显示区域、不透明度、速度、时间偏移等参数调节。
- 支持弹幕类型过滤、关键词屏蔽和正则屏蔽。
- 支持从 `空降`、`跳伞`、`指路`、`传送`、`跳转` 等弹幕关键词中识别跳转提示。
- 支持相似弹幕动态区间合并，可调相似度、最小数量、相邻间隔和最大跨度，显示为 `弹幕x10` 一类计数弹幕。
- 支持高密度保护，同刻发送按每秒局部密度削峰，最大加载作为独立全局上限，并支持滑条或数字输入。
- 使用 `localStorage` 持久化保存设置、匹配缓存和偏好缓存。
...
6. 悬停控制栏的设置按钮，可从 BV/视频链接、`b23` 短链或正版番剧 `ep`/`ss` 链接导入 B 站弹幕。
7. 如果需要过滤内容，可以添加关键词规则或正则规则；如果需要压缩刷屏内容，可以在 `过滤` 页调整相似合并。

## 配置说明

- 脚本会把设置、API 配置、匹配缓存和系列偏好写入浏览器 `localStorage`。
- 可以在面板里填写自定义 API 前缀，用于接入你自己的弹弹 Play 兼容接口。
- 当直连不可用时，脚本会自动尝试内置代理候选地址。
- B 站导入绑定按 AniCh 路由保存；BV `?p=` 和正版番剧 `ep`/`ss` 输入可以生成同季自动恢复链。
- B 站导入会与已有来源做跨来源去重：完全相同弹幕会按精确指纹去重，归一化后同文同类型且时间相差 `0.2s` 内的弹幕会按模糊重复处理；当足够多重叠弹幕显示出稳定来源时间偏移时，该偏移只用于去重判断，不改变实际播放时间。运行概括的来源桶会显示原始、并入、去重和对齐偏移。
